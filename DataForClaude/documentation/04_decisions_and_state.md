...
| D12 | 2026-09-14 | **Start a linear-regression version of SpaceTravLR** (`metab_processing/LinearRegression/`): each **cell-type annotation** gets its own β per (modulator, target gene) via a plain per-cell-type OLS, instead of the per-cell CNN. Step 1 (done): a preprocess + compute-x + save function that writes **ONE adata** (imputed-count layer + x data) to `<dataset>/LinearRegression/x_adata.h5ad` — **no `spacetravlr_output/` tree, no CellOracle/NicheNet networks** (those aren't needed for the stored x and are deferred to the regression step). **Store the x as target-agnostic *building blocks*, NOT per-gene design matrices** (avoids the cells×M×G blowup Foster flagged): `obsm['x_metab']` (metabolite scores) + `obsm['received_ligands']` (unfiltered CellChat diffusion) + labels in `uns`; L–R/L–TF/TF factors reconstruct losslessly at regression time as elementwise products with raw `imputed_count`. | `build_x.py::build_x_adata(adata, out_path, *, annot, metabolites, radius=100, contact_distance=30, scale_factor=100, min_cells_for_magic=16, layer='imputed_count')`. Preprocessing is **in-house** (`_preprocess`: reuses `scale_adata` [scales spatial so `radius` is in the right units], `encode_labels`→`cell_type_int`, `impute_clusterwise`→`imputed_count`) — i.e. `process_adata_` MINUS the UMAP (CellOracle-only) MINUS the file-tree writes. Rev 2 (2026-09-15) **dropped the earlier `SpaceShip.setup_` reuse** because it created the whole `spacetravlr_output/betadata/...` tree Foster didn't want. The x uses `beta_analysis.metab_x_to_adata` + `init_received_ligands`. **COMMOT stays OFF** (D7): with no thresholds, `received_ligands == received_ligands_tfl`, so the single stored (unfiltered) block matches training's L–R/L–TF/metab source; if `cell_thresholds` is present `add_x_data` warns. The per-cell-type OLS solver + tidy output is the next step. |
...
- **`build_x.py`** (no `src/` edits, no new deps): `build_x_adata` does lean **in-house** preprocessing
  (`_preprocess`: `scale_adata` + `encode_labels`→`cell_type_int` + `impute_clusterwise`→`imputed_count`;
  no UMAP, no file tree) then `add_x_data`, and writes ONE adata. `add_x_data` stores `obsm['x_metab']`
  (via tested `beta_analysis.metab_x_to_adata`) + `obsm['received_ligands']` (fresh unfiltered diffusion
  at `radius`, `cell_threshes=None`) + label lists in `uns`; big `uns` diffusion frames popped so the
  h5ad doesn't balloon.
  - **Rev 2 (2026-09-15):** dropped the initial `SpaceShip.setup_` reuse — it produced the whole
    `spacetravlr_output/{input_data,betadata,logs}/` + CellOracle/NicheNet tree, but the stored x only
    needs MAGIC imputation + the shipped CellChat DB. Networks are deferred to the regression step.
    Also added `impute_clusterwise(min_cells_for_magic=16)`: a cell-type below 16 cells keeps raw counts
    instead of crashing MAGIC's kNN (was `n_neighbors=16 > n_samples`).
