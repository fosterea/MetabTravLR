...
| D12 | 2026-09-14 | **Start a linear-regression version of SpaceTravLR** (`metab_processing/LinearRegression/`): each **cell-type annotation** gets its own β per (modulator, target gene) via a plain per-cell-type OLS, instead of the per-cell CNN. Step 1 (done): a preprocess + compute-x + save function. **Store the x as target-agnostic *building blocks*, NOT per-gene design matrices** (avoids the cells×M×G blowup Foster flagged): `obsm['x_metab']` (metabolite scores) + `obsm['received_ligands']` (unfiltered CellChat diffusion) + labels in `uns`; L–R/L–TF/TF factors reconstruct losslessly at regression time as elementwise products with raw `imputed_count`. | `build_x.py::build_x_adata(adata, out_path, *, annot, metabolites, radius=100, contact_distance=30, scale_factor=100, run_commot=False, reuse_setup=True)` reuses `SpaceShip.setup_` for imputation+networks (it *is* SpaceTravLR's init) and `beta_analysis.metab_x_to_adata` + `init_received_ligands` for the x. **COMMOT stays OFF** (D7): with `run_commot=False`, `received_ligands == received_ligands_tfl`, so the single stored (unfiltered) block matches training's L–R/L–TF/metab source. If `cell_thresholds` is present it warns (the block is unfiltered; COMMOT L–R filtering not reflected). Output for UC slice 4 → `<dataset>/LinearRegression/x_adata.h5ad`. The per-cell-type OLS solver + tidy output is the next step. |
...
- **`build_x.py`** (97 lines, no `src/` edits, no new deps): `build_x_adata` reuses `SpaceShip.setup_`
  (impute + CellOracle/NicheNet networks — SpaceTravLR's init) then `add_x_data`; `reuse_setup=True`
  skips the expensive MAGIC+CellOracle rebuild if a processed `_adata.h5ad` already exists.
  `add_x_data` stores `obsm['x_metab']` (via tested `beta_analysis.metab_x_to_adata`) +
  `obsm['received_ligands']` (fresh unfiltered diffusion at `radius`, `cell_threshes=None`) + label
  lists in `uns`; big `uns` diffusion frames popped so the h5ad doesn't balloon.
