...
| D12 | 2026-09-14 | **Start a linear-regression version of SpaceTravLR** (`metab_processing/LinearRegression/`): each **cell-type annotation** gets its own β per (modulator, target gene) via a plain per-cell-type OLS, instead of the per-cell CNN. Step 1: a function that writes **ONE adata** (imputed-count layer + x data) to `<dataset>/LinearRegression/x_adata.h5ad`. **Rev 3 (2026-09-15, Foster's explicit call): store the FULL per-gene design matrix for ALL FOUR groups (TF + L–R + L–TF + metab), not building blocks.** This **supersedes** the Rev-1/Rev-2 "target-agnostic building blocks" choice below — Foster: *"I want all the factor groups stored including LR, TF, L-TF … We are going to train on all the groups."* Per focus gene, store `obsm['x_<gene>']` (= the estimator's `train_df` minus the target col) + `uns['x_<gene>_cols']`; `uns['x_genes']` lists them. | `build_x.py::build_x_adata(adata, out_path, *, focus_genes, metabolites=None, setup_dir=None, annot='cell_type', cluster_annot='cell_type_int', run_commot=False, radius=300, contact_distance=50, scale_factor=1, tf_ligand_cutoff=0.01, receptor_thresh=0.01, layer='imputed_count')` + `store_design_matrices(...)`. **Reuses the real estimator's `init_data()` per gene** (no reimplemented factor math) — needs the networks, so it reuses an existing `setup_dir/spacetravlr_output` (or runs `SpaceShip.setup_`, whose file tree Foster now accepts as "junk"). Params come from `betadata/run_params.json` when present (authoritative; note it does NOT persist `receptor_thresh`), else the training defaults (300/50/1/0.01 — NOT the estimator's 100/30/100). Clears stale `received_ligands*` before the loop so a COMMOT cache can't be silently reused; drops `spatial_maps`/diffusion frames before writing. **Scale caveat:** per-gene dense matrices are `O(cells × M_g × n_genes)` — fine for the focus-gene lists + small slices we use; revisit if pushing 100k–1M cells × many genes. The per-cell-type OLS solver + tidy output is the next step. |
...
- **`build_x.py`** (no `src/` edits, no new deps). History of the x-storage design:
  ...
  - **Rev 3 (2026-09-15) — CURRENT:** Foster clarified he wants **all four factor groups stored per
    gene**, ready to train on. So `build_x_adata` now reuses the real estimator's `init_data()` per
    focus gene and stores each gene's full design matrix (`obsm['x_<gene>']` = TF + `lig$rec` +
    `lig#tf` + `metab@` columns; `uns['x_<gene>_cols']`; `uns['x_genes']`). This needs the CellOracle/
    NicheNet networks, so it reuses (or builds via `setup_`) a `spacetravlr_output` — the file tree is
    accepted as "junk"; the deliverable is the single `LinearRegression/x_adata.h5ad`. `store_design_matrices`
    is the testable core (takes `grn`+`tflinks`, validated against `init_data()` as ground truth). Params
    come from `run_params.json` when present, else the training defaults (300/50/1/0.01). Clears any stale
    `received_ligands*` before the loop (COMMOT-cache footgun the sibling `compute_metab_x` also guards);
    strips `spatial_maps`/diffusion frames before write.
  ...
  - Dev→review loop (metab-dev + metab-review, plan pre-validated by a Plan agent). Review caught a
    stale-diffusion/COMMOT blocker and a dead `receptor_thresh` run_params override (not persisted by
    `oracles.py`); both fixed.
